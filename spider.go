...
var _ core.ICollector = &Collector{}
...
func (c *Collector) AllowURLRevisit() bool {
	return c.allowURLRevisit
}

//NewID requestid
func (c *Collector) NewID() uint32 {
	return atomic.AddUint32(&c.requestCount, 1)
}

//UserAgent value
func (c *Collector) UserAgent() string {
	return c.userAgent
}
...
func (c *Collector) Init() {
	...
	c.backend = &core.HttpBackend{}
	jar, _ := cookiejar.New(nil)
	c.backend.Init(jar)
	c.wg = &sync.WaitGroup{}
	c.lock = &sync.RWMutex{}
	//todo c.robotsMap = make(map[string]*robotstxt.RobotsData)
...
//Scrape todo add comment
func (c *Collector) Scrape(u, method string, depth int, requestData io.Reader,
	ctx *core.Context, hdr http.Header, checkRevisit bool) error {
...
func (c *Collector) fetch(u, method string, depth int, requestData io.Reader,
	ctx *core.Context, hdr http.Header, req *http.Request) error {
	defer c.wg.Done()
	if ctx == nil {
		ctx = core.NewContext()
	}

	request := core.NewRequest(method, req.URL, requestData,
		ctx, &req.Header, atomic.AddUint32(&c.requestCount, 1), depth, c)
	...
	for _, cc := range c.htmlCallbacks {
		...
		doc.Find(cc.Selector).Each(func(_ int, s *goquery.Selection) {
			for _, n := range s.Nodes {
				e := query.NewHTMLElementFromSelectionNode(resp, s, n, i)
				i++
				//todo
				// if c.debugger != nil {
